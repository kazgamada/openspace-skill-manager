...
## 14. アセットライブラリ UI — 魅力的な商品展示と循環表示

### 14.1 設計思想

技術仕様の羅列ではなく「**これを入れると何が変わるか**」を前面に出す。  
App Store・Netflix・Product Hunt を参考にした **ベネフィット訴求型 UI** を採用する。

| 従来の表示（Before） | 新しい表示（After） |
|---------------------|-------------------|
| `format-on-save` — フックの説明 | **「保存するたびに自動整形。手動 prettier は今日で終わり。」** |
| `code-reviewer` — スキル | **「レビュアーがいなくても、AI が即座にコードの問題を指摘」** |
| `commit.md` — コマンド | **「`/commit` と打つだけで、diff を読んで最適なメッセージを生成」** |

---

### 14.2 ページ全体レイアウト（`/library`）

```
┌──────────────────────────────────────────────────────────────────┐
│  [A] ヒーローカルーセル（自動回転・フルワイド）                      │
│      キュレーション済み or トレンド急上昇アセットを 5〜8 件ローテーション│
├──────────────────────────────────────────────────────────────────┤
│  [B] クイックフィルターバー                                         │
│      [全て] [スキル] [フック] [コマンド] [エージェント] [MCP] [指示書] │
│      ＋ [今日のピック] [急上昇] [新着] [セット]                      │
├──────────────────────────────────────────────────────────────────┤
│  [C] パーソナライズ行「あなたへのおすすめ」            → 横スクロール │
├──────────────────────────────────────────────────────────────────┤
│  [D] カテゴリ別 Netflix 行（各 10〜12 件）            → 横スクロール │
│      • 🔥 今週の急上昇                                             │
│      • ✨ 今日のキュレーター厳選                                    │
│      • 🤖 エージェント & MCP セット                                 │
│      • 🛠  開発ワークフロー強化パック                                │
...
│  手動 prettier は今日で終わり。                                     │
│                                                                  │
│  format-on-save                          ★ 4.8   👥 2,341 人が使用 │
│  by kazgamada                                                    │
│                                                                  │
│  ✅ 安全確認済み  📄 MIT ライセンス  ⏱ インストール 10 秒           │
│                                                                  │
│  [プレビューを見る]  [ワンクリックで追加]                             │
└─────────────────────────────────────────────────────────────────┘
    ←  ─────────────────────────────────────────────────  →
...
| パラメータ | 値 | 説明 |
|-----------|----|----|
| 表示件数 | 5〜8 件 | 当日のキュレーションリストから選出 |
| 自動送り間隔 | 6 秒 | ホバー中は停止 |
| 選出ロジック | `isCurated=true` を優先、次に `trendScore` 上位 | |
| 更新タイミング | 毎日 0 時にサーバーサイドで差し替え | `daily_hero_picks` テーブルに保存 |
| トランジション | フェードイン + 左スライド（300ms） | framer-motion |
...
```
[全て 3,740] [スキル 2,847] [フック 312] [コマンド 189]
[エージェント 94] [MCP 67] [指示書 231]
─────────────────────────────────────────────────
[🔥 急上昇] [✨ 今日のピック] [🆕 新着 7日] [📦 セット]
```

- タブ切り替えと同時にグリッドが **アニメーション付きで差し替わる**
- 「急上昇」選択時はカードに炎バッジ + スター増加数を表示
- カウント数は `assets/index.json` から取得（API 不要）

---

### 14.5 [C][D] カルーセル行（Netflix スタイル）
...
- カード幅: 220px（tile-lg）
- 1行あたり表示数: 4〜6枚（画面幅に応じて）
- `embla-carousel-react`（既存依存）で実装
- 矢印クリックで 3 枚ずつ送り
- **ホバー時にカードが 1.03× に拡大** + 詳細プレビューがポップアップ

#### 行の定義と選出ロジック

| 行名 | 絵文字 | 選出条件 | 更新頻度 |
|------|--------|---------|---------|
| 今週の急上昇 | 🔥 | `trendScore` 上位・全タイプ混在 | 1時間 |
| 今日のキュレーター厳選 | ✨ | `isCurated=true` かつ当日選出 | 毎日 0 時 |
| あなたへのおすすめ | 👤 | インストール済みアセットと同カテゴリ上位 | セッション毎 |
| エージェント & MCP セット | 🤖 | `type=agent OR mcp`・`finalScore` 上位 | 6時間 |
| 開発ワークフロー強化 | 🛠 | `category=development`・全タイプ | 6時間 |
| フロントエンド向け | 📝 | `tags` に `react/vue/css/frontend` 含む | 6時間 |
| 新着（過去 7 日） | 🆕 | `collectedAt > now-7d` | 1時間 |
| 最もインストールされた | 👑 | `installCount` 上位 | 6時間 |
...
```
┌────────────────────────────────────┐
│ [⚡ フック]          [🔥 急上昇 +142] │  ← タイプバッジ + トレンドバッジ
│                                     │
│ 保存で自動整形。                      │  ← ベネフィットヘッドライン（20字以内）
│ もう手動は不要。                      │    LLM が自動生成
│                                     │
│ format-on-save                      │  ← 技術名（小さく）
│ by kazgamada                        │
│                                     │
│ ████████████░░  87pt                │  ← スコアバー
│                                     │
│ [prettier] [PostToolUse] [Write]    │  ← タグ（最大3）
│                                     │
│ 👥 2,341  ★ 4.8  ✅ 安全            │  ← 社会的証明
│                                     │
│ [プレビュー]        [+ 追加]          │
...
```
┌─────────────────────────────────────────────────────────────────┐
│ ⚡  format-on-save  [フック] [急上昇🔥]              ★4.8  👥2,341│
│    保存するたびに自動整形。手動 prettier は今日で終わり。             │
│    [prettier][PostToolUse][Write]    ████████░  87pt  [+追加] │
└─────────────────────────────────────────────────────────────────┘
```

**③ tile-sm（コンパクト）— カルーセル行**
...
#### ベネフィットヘッドラインの自動生成ルール

アセット収集時に LLM（Forge API）が `content` を読み込み、以下フォーマットで生成して `meta.json` に保存：

```
benefitHeadline: "〇〇するだけで、△△が不要になる。"（20字以内）
benefitDetail:   "具体的な効果を 1〜2 文で説明"（60字以内）
useCases:        ["usecase1", "usecase2", "usecase3"]  ← 3 つの使用シーン
targetUsers:     ["フロントエンド開発者", "TypeScriptユーザー"]
```
...
| タイプ | ヘッドライン | サブテキスト |
|--------|------------|------------|
| フック | 「保存で自動整形。手動 prettier は今日で終わり。」 | ファイル保存のたびに自動フォーマット。コードレビューでスタイル指摘がゼロに。 |
| スキル | 「レビュアー不在でも、コードの問題を即指摘」 | diff を読んで型・ロジック・セキュリティの観点でレビューコメントを生成。 |
| コマンド | 「`/commit` 一言で、差分から最適なコミットメッセージ」 | 変更内容を解析して Conventional Commits 形式で自動生成。 |
| エージェント | 「テスト専門 AI が、カバレッジ不足を自動検出」 | テストファイルを解析し、未テストのケースとテストコードを提案。 |
| MCP | 「GitHub を自然言語で操作。PR もレビューも会話で」 | Issue 作成・PR マージ・コードレビューを Claude との対話で実行。 |
| CLAUDE.md | 「Next.js チーム全員が同じ AI コンテキストを共有」 | プロジェクトの規約・構造・コマンドを Claude が自動で把握。 |

---

### 14.7 アセット詳細プレビューモーダル

カード上の「プレビュー」ボタンで開く。**インストール前に価値を十分に伝える。**
...
│  ─────────────────────────────────────────────────────────────  │
│                                                                  │
│  👥 2,341 人が使用   ★ 4.8（187件）   🔥 今週 +142   ✅ 安全確認済 │
│                                                                  │
│  ┌── こんな人におすすめ ──────────────────────────────────────┐   │
│  │  ✓ チームで prettier を使っている                          │   │
│  │  ✓ コードレビューでスタイル指摘を減らしたい                 │   │
│  │  ✓ 保存ショートカットをよく使う                            │   │
│  └────────────────────────────────────────────────────────────┘  │
│                                                                  │
│  ┌── 使用シーン ────────────────────────────────────────────┐   │
│  │  1. TypeScript プロジェクトで保存時に自動整形              │   │
│  │  2. チーム全員のフォーマットを統一                         │   │
│  │  3. CI の lint エラーをゼロにする                         │   │
│  └────────────────────────────────────────────────────────────┘  │
│                                                                  │
│  ┌── 設定内容（インストールされるもの）────────────────────────┐   │
│  │  イベント: PostToolUse → Write|Edit                       │   │
│  │  コマンド: prettier --write "$FILE"                       │   │
│  │  ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─                   │   │
│  │  .claude/settings.json に追記されます            [コピー] │   │
│  └────────────────────────────────────────────────────────────┘  │
│                                                                  │
│  ┌── よく一緒に使われる ────────────────────────────────────┐   │
│  │  [⚡ eslint-on-save]  [🤖 code-reviewer]  [📝 commit]   │   │
│  └────────────────────────────────────────────────────────────┘  │
│                                                                  │
│  [← 戻る]          [★ お気に入り]          [✚ ワンクリックで追加]   │
└──────────────────────────────────────────────────────────────────┘
...
| タブ | 内容 |
|------|------|
| 概要 | ベネフィット・おすすめ対象・使用シーン（上記デザイン） |
| ファイル内容 | シンタックスハイライト付きコードビューアー |
| レビュー | 評価（1〜5星）+ コメント一覧 |
| 変更履歴 | バージョン差分・更新日時・変更内容 |

---

### 14.8 セット（バンドル）機能

関連アセットをセットにして「まとめてインストール」できる機能。

```
┌──────────────────────────────────────────────────────────────────┐
│  📦 コードレビュー強化セット                          [キュレーター選定]│
│  「PR 前の品質チェックを完全自動化。レビュー指摘を 70% 削減。」        │
│                                                                  │
│  含まれるアセット（4点）:                                           │
│  ┌────────────────────────────────────────────────────────────┐ │
│  │ 🎯 [スキル]   code-reviewer     — コードレビュー実行         │ │
│  │ ⚡ [フック]   eslint-on-save     — 保存時に自動 lint         │ │
│  │ 💬 [コマンド] review-pr         — PR 全体をまとめてレビュー  │ │
│  │ 📄 [CLAUDE.md] review-guideline — レビュー観点を AI に共有  │ │
│  └────────────────────────────────────────────────────────────┘ │
│                                                                  │
│  👥 891 セット導入済み   ★ 4.9                                    │
│                                                                  │
...
- `curated_lists` テーブルの `type="set"` レコードとして管理
- 管理者が UIから作成（セット名・説明・含むアセット ID 群・ヘッドラインコピー）
- ユーザー投稿セットも可能（モデレーション後公開）
- セット全体のスコア = 含むアセットの `finalScore` 平均

---

### 14.9 循環表示・コンテンツ鮮度の仕組み

#### デイリーローテーション

毎日 0 時に `daily_hero_picks` テーブルを更新するジョブが動く。

```
選出アルゴリズム:
1. isCurated=true のアセットを候補に
2. 過去 7 日以内に hero に表示されたものを除外
3. finalScore 降順で上位 8 件を選出
4. ただし同じ assetType が 3 件以上にならないようにバランス調整
5. DB に保存 → CDN キャッシュをパージ
```

#### ウィークリーキュレーション

毎週月曜に管理者パネルから「今週のピックアップ」を手動選定 + ヘッドラインコピーを編集。  
→ ヒーローカルーセルの 1 枚目に固定表示。

#### トレンドバッジの更新

```
⏰ 1時間毎:
  - 直近 24h の installCount 増加数を計算
  - 増加率 > 50% → 🔥 急上昇バッジ
  - 増加率 > 20% → 📈 注目バッジ
  - 7日以内初登場 → 🆕 新着バッジ
  - バッジは meta.json に反映 → フロントエンドはポーリング不要（index.json 参照）
```

#### 「あなたへのおすすめ」のパーソナライズ

```
ロジック（サーバーサイド、ログイン時のみ）:
1. ユーザーのインストール済みアセットのタグ・カテゴリを集計
2. 同タグ・カテゴリで未インストールのアセットを finalScore 順で抽出
3. 「よく一緒に使われる」（co-install 分析）で補完
4. キャッシュ: 30分 / ユーザー
```
...
インストール中...（スピナー 1〜2 秒）
    ↓
✅ インストール完了！
「.claude/commands/commit.md に追加されました」
[使い方を見る]  [元に戻す]
```

**セキュリティ警告ありのアセット（フック・MCP）**
...
│ prettier --write "$FILE"                          │
└──────────────────────────────────────────────────┘
外部コマンド実行を含みます。内容を確認してください。

[キャンセル]   [内容を確認]   [理解した上でインストール]
```

---

### 14.11 スコア内訳の可視化（「なぜ人気？」）

カード・モーダルに「人気の理由」を視覚的に表示する。

```
なぜ人気？

GitHub    ████████░░  78  (stars: 1,284 / forks: 92)
X 言及    ██████████  94  (いいね計: 3,421 / RT: 891)
OSM評価   ███████░░░  68  (★4.8 / 187件)
品質      █████████░  88  (構造: 完全 / 長さ: 適切)
─────────────────────────────────────
総合      ████████░░  83
...
|--------|----|----- |
| `id` | SERIAL | 主キー |
| `date` | DATE UNIQUE | 対象日付 |
| `assetIds` | TEXT | JSON 配列（順序付き） |
| `headlines` | TEXT | JSON — `{assetId: benefitHeadline}` 上書き用 |
...
|--------|----|----- |
| `userId` | INT | ユーザー |
| `assetId` | VARCHAR(64) | アセット |
| `createdAt` | TIMESTAMP | お気に入り登録日 |

#### `asset_co_installs`（よく一緒に使われる分析）

| カラム | 型 | 説明 |
|--------|----|----- |
| `assetIdA` | VARCHAR(64) | アセット A |
| `assetIdB` | VARCHAR(64) | アセット B |
| `coInstallCount` | INT | 同時インストール数 |
| `updatedAt` | TIMESTAMP | 集計日時 |

---

### 14.13 新規 tRPC エンドポイント（UI 向け）

| プロシージャ | 説明 |
|------------|------|
| `library.getHeroPicks` | 当日のヒーローカルーセル取得 |
| `library.getRow` | 行 ID 指定でカルーセル行を取得 |
| `library.getRecommended` | パーソナライズおすすめ取得 |
| `library.getSet` | セット詳細取得 |
| `library.installSet` | セット一括インストール |
| `library.rateAsset` | 評価投稿（1〜5星 + コメント） |
| `library.favoriteAsset` | お気に入り登録/解除 |
| `library.getCoInstalls` | よく一緒に使われるアセット取得 |
| `admin.setHeroPicks` | ヒーロー選出の手動上書き |
| `admin.createSet` | セット（バンドル）作成 |
...
| Phase | タスク |
|-------|--------|
| **Phase 2** | `community_assets` 拡張（benefitHeadline・targetUsers・useCases フィールド）、収集時の LLM ベネフィット生成 |
| **Phase 3** | `/library` ページ新設、ヒーローカルーセル、クイックフィルターバー、tile/list カード全タイプ対応 |
| **Phase 3** | `daily_hero_picks` テーブル・デイリーローテーションジョブ |
| **Phase 4** | Netflix 行（カテゴリ別カルーセル）、パーソナライズおすすめ、プレビューモーダル |
| **Phase 4** | `asset_ratings` + `asset_favorites` + 評価 UI |
| **Phase 5** | セット（バンドル）機能、`asset_co_installs` 分析、スコア内訳可視化 |

---

*本要件定義書は 2026-04-09 に更新されました（セクション 14 追記）。*
