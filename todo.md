...
- [ ] ダッシュボード: 全情報集約型レイアウトに刷新（統計・通知・スキル一覧・同期ログ・広場トップ・進化提案を1画面に）
- [ ] ダッシュボード: 修復(24h)・派生(24h)カードにツールチップで仕組みを説明
- [ ] 初期設定ウィザード: 修復・派生のメカニズム説明ステップを追加（Step 0 として冒頭に配置）
- [ ] 初期設定ウィザード: 修復・派生の有効化トグル・閾値設定をウィザード内に追加
- [ ] マイスキル: スキル一覧に新規・修復済・派生の3種バッジを追加（DBカラム skill_badge を参照）
- [ ] DBスキーマ: my_skills テーブルに badge カラム（new/repaired/derived/null）を追加
- [ ] 社内説明ドキュメント v3: ダッシュボード設計・修復派生の仕組み・バッジ仕様を追記
